...
if [ "$(id -u)" -eq 0 ]; then
    IS_ROOT=1
    [ "${NUTS_CONTAINER_VERBOSE:-0}" = "1" ] &&  echo "Detected Root Mode"
fi

# --- Create non-root user if running as root ---
if [ $IS_ROOT -eq 1 ]; then
    if ! id "$NUTS_USERNAME" >/dev/null 2>&1; then
        useradd -m -s /bin/bash "$NUTS_USERNAME"
        [ "${NUTS_CONTAINER_VERBOSE:-0}" = "1" ] && echo "User '$NUTS_USERNAME' created."
    else
        [ "${NUTS_CONTAINER_VERBOSE:-0}" = "1" ] && echo "User '$NUTS_USERNAME' already exists."
    fi
    TARGET_USER=$NUTS_USERNAME
    TARGET_HOME="/home/$NUTS_USERNAME"
else
    TARGET_USER=$(whoami)
    TARGET_HOME="$HOME"
    [ "${NUTS_CONTAINER_VERBOSE:-0}" = "1" ] && echo "Running as non-root user: $TARGET_USER"
fi
...
if [ $IS_ROOT -eq 1 ]; then
    # Forward PATH and arguments to su login shell
    su - "$NUTS_USERNAME" -c '
    set -e
    NUTS_VERSION=$1
    shift
    ...
    source ~/.bashrc
    exec bash
    ' _ "$NUTS_VERSION" "$JAVA_BIN:$PATH" "${NUTS_ARGS[@]}"
else
