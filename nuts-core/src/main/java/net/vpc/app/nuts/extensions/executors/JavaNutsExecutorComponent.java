...
public class JavaNutsExecutorComponent implements NutsExecutorComponent {
    ...
    public void exec(NutsExecutionContext executionContext) throws IOException {
        ...
        List<NutsFile> nutsFiles = executionContext.getWorkspace().fetchWithDependencies(executionContext.getNutsFile().getId().toString(), true, NutsUtils.EXEC_DEPENDENCIES_FILTER, executionContext.getSession().copy().setTransitive(true));
        ...
        if (nutsFiles.size() > 0) {
            ...
            for (NutsFile nutsFile : nutsFiles) {
                ...
                if(nutsFile.getId().isSameFullName(executionContext.getNutsFile().getId())){
                    mainClass = IOUtils.getMainClass(nutsFile.getFile());
                }
            }
            args.add(sb.toString());
        }
        if(mainClass==null){
            throw new IllegalArgumentException("Missing Main-Class in Manifest for "+executionContext.getNutsFile().getId());
        }
        args.add(mainClass);
