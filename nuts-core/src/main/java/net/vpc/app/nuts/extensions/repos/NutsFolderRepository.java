...
//                        }
            ...
            Iterator<NutsId> allVersions = findVersions(id, CoreVersionUtils.createFilter(versionString), session);
            ...
            if (session.isTransitive()) {
                ...
                for (NutsRepository repo : getMirrors()) {
                    ...
                    if (sup > 0) {
                        ...
                        if (toFile!=null) {
                            Boolean executableJar = CorePlatformUtils.getExecutableJar(toFile);
...
//        }
    ...
    protected NutsFile getLocalGroupAndArtifactAndVersionFile(NutsId id, boolean desc) throws IOException {
        ...
        NutsDescriptor d = CoreNutsUtils.parseNutsDescriptor(localNutDescriptorFile.getFile());
        if (d == null) {
            return null;
