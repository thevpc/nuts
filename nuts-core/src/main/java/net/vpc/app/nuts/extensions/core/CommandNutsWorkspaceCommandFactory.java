...
class CommandNutsWorkspaceCommandFactory implements NutsWorkspaceCommandFactory {
    ...
    public void configure(NutsWorkspaceCommandFactoryConfig config) {
        ...
        if (p != null) {
            ...
            listCommand = slistCommand == null ? new String[0] : NutsArgumentsParser.parseCommandLine(slistCommand);
    ...
    private String[] validateCommand(String command){
        ...
        String[] commandArr=NutsArgumentsParser.parseCommandLine(command);
    ...
    public NutsWorkspaceCommandConfig findCommand(String name, NutsWorkspace workspace) {
        if(findCommand.length>0 && execCommand.length>0){
            ...
            NutsCommandExecBuilder exec = workspace.createExecBuilder().setCommand(fc)
//                        .setExecutorOptions("--show-command")
            ...
            if (r == 0) {
                return new NutsWorkspaceCommandConfig()
                        ...
                        .setId(CoreNutsUtils.parseNutsId(ec[0]))
                        .setName(name).setCommand(Arrays.copyOfRange(ec,1,ec.length));
            }
        }
        return null;
    }
    ...
    public List<NutsWorkspaceCommandConfig> findCommands(NutsWorkspace workspace) {
        ...
        if(listCommand.length>0) {
            NutsCommandExecBuilder b = workspace.createExecBuilder().setCommand(listCommand)
                    .setRedirectErrorStream()
                    .grabOutputString();
