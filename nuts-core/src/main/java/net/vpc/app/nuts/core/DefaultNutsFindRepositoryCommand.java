...
import net.vpc.app.nuts.NutsFindRepositoryCommand;
 ...
 * @author vpc
 */
public class DefaultNutsFindRepositoryCommand extends NutsRepositoryCommandBase<NutsFindRepositoryCommand> implements NutsFindRepositoryCommand {

    private static final Logger LOG = Logger.getLogger(DefaultNutsFindRepositoryCommand.class.getName());

    private NutsIdFilter filter;
    private Iterator<NutsId> result;

    public DefaultNutsFindRepositoryCommand(NutsRepository repo) {
        super(repo);
    }
    ...
    public NutsFindRepositoryCommand run() {
        ...
        getRepo().security().checkAllowed(NutsConstants.Rights.FETCH_DESC, "find");
        ...
        try {
            ...
            if (getSession().isIndexed() && xrepo.getIndexStoreClient() != null && xrepo.getIndexStoreClient().isEnabled()) {
                ...
                try {
                    o = xrepo.getIndexStoreClient().find(filter, getSession());
            ...
            result=xrepo.findImpl(filter, getSession());
    ...
    @Override
    public NutsFindRepositoryCommand filter(NutsIdFilter id) {
        return setFilter(id);
    }

    @Override
    public NutsFindRepositoryCommand setFilter(NutsIdFilter filter) {
        this.filter = filter;
        return this;
    }
