...
import net.vpc.app.nuts.core.util.app.options.ExtensionNonOption;
...
public class DefaultWorkspaceCommandLine implements NutsCommandLine {
    ...
    protected List<NutsArgument> expanded = new ArrayList<>();
                        ...
                        if (p.getName().getString().equals(name)) {
                            if (p.isNegated()) {
                                if (p.isKeyValue()) {
                                    ...
                                    skip();
                                    return newArgument(p.getName() + eq + (!x));
                                } else {
                                    skip();
                                    return newArgument(p.getName() + eq + (false));
                                }
                                ...
                                skip();
                                return newArgument(p.getName() + eq + (true));
                            }
                        }
                        break;
                    }
        ...
        if (hasNext() && get().isNonOption()) {
            ...
            if (expectValue) {
                if (p.isKeyValue()) {
                    ...
                    if (r2 != null && !r2.isOption()) {
                        skip(2);
                        return newArgument(p.getString() + "=" + r2.getString());
                    }
                }
            } else {
                skip();
    ...
    public NutsArgument get(int i) {
        ...
        while (!args.isEmpty() && i >= expanded.size()) {
    ...
    @Override
    public int length() {
        return expanded.size() + args.size();
    }
    ...
    public String[] toArray() {
        ...
        for (NutsArgument nutsArgument : expanded) {
    ...
    public class ArgumentReader {
        ...
        public boolean readAll(String... vals) {
            return DefaultWorkspaceCommandLine.this.readAll(repeatable, vals);
        }
