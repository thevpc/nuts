...
public class NHomeLocation implements NEnum {
    ...
    public static NOptional<NHomeLocation> parse(String value) {
        ...
        if(value.isEmpty()){
            return NOptional.ofEmpty(s -> NMsg.ofCstyle("%s is empty", NHomeLocation.class.getSimpleName()));
        }
        ...
        if (i >= 0) {
            ...
            if (osf == null) {
                if (!s1.equals("system") && s1.length() > 0) {
                    return NOptional.ofError(s -> NMsg.ofCstyle(NHomeLocation.class.getSimpleName() + " invalid value : %s", finalValue));
                }
            }
            if (loc == null) {
