...
import net.thevpc.nuts.reserved.NutsReservedIOUtils;
 ...
 * @since 0.8.1
 */
public final class NutsPlatformUtils {
    ...
    public static String getCustomPlatformHomeFolder(NutsOsFamily platformOsFamily, NutsStoreLocation location, Map<NutsHomeLocation, String> homeLocations) {
        ...
        s = NutsStringUtils.trim(System.getProperty("nuts.home." + locationName + "." + platformOsFamily.id()));
        if (!s.isEmpty()) {
            return s/* + "/" + workspaceName*/;
        }
        s = NutsStringUtils.trim(System.getProperty("nuts.export.home." + locationName + "." + platformOsFamily.id()));
        if (!s.isEmpty()) {
            return s/* + "/" + workspaceName*/;
        }
        if (homeLocations != null && homeLocations.size() > 0) {
            NutsHomeLocation key = NutsHomeLocation.of(platformOsFamily, location);
            s = NutsStringUtils.trim(homeLocations.get(key));
            if (!s.isEmpty()) {
                return s/* + "/" + workspaceName*/;
            }
    ...
    public static String getWorkspaceLocation(NutsOsFamily platformOsFamily, boolean global, String workspaceName) {
        ...
        if (global) {
            ...
            switch (platformOsFamily) {
                ...
                default: {
                    String val = NutsStringUtils.trim(System.getenv("XDG_CONFIG_HOME"));
                    if (!val.isEmpty()) {
    ...
    public static String getDefaultPlatformHomeFolder(NutsOsFamily platformOsFamily, NutsStoreLocation location, boolean global, String workspaceName) {
        ...
        if (global) {
            String s = null;
            s = NutsStringUtils.trim(System.getProperty("nuts.home.global." + locationName + "." + platformOsFamily.id()));
            if (!s.isEmpty()) {
                return s + "/" + workspaceName;
            }
            s = NutsStringUtils.trim(System.getProperty("nuts.export.home.global." + locationName + "." + platformOsFamily.id()));
            if (!s.isEmpty()) {
            ...
            switch (location) {
                ...
                case LIB: {
                    switch (platformOsFamily) {
                        ...
                        default: {
                            String val = NutsStringUtils.trim(System.getenv("XDG_DATA_HOME"));
                            if (!val.isEmpty()) {
                ...
                case LOG: {
                    switch (platformOsFamily) {
                        ...
                        default: {
                            String val = NutsStringUtils.trim(System.getenv("XDG_LOG_HOME"));
                            if (!val.isEmpty()) {
                                return val + "/nuts/" + workspaceName;
                            }
                ...
                case RUN: {
                    switch (platformOsFamily) {
                        ...
                        default: {
                            String val = NutsStringUtils.trim(System.getenv("XDG_RUNTIME_DIR"));
                            if (!val.isEmpty()) {
                                return val + "/nuts/" + workspaceName;
                            }
                ...
                case CACHE: {
                    switch (platformOsFamily) {
                        ...
                        default: {
                            String val = NutsStringUtils.trim(System.getenv("XDG_CACHE_HOME"));
                            if (!val.isEmpty()) {
                                return val + "/nuts/" + workspaceName;
                            }
        ...
        for (NutsStoreLocation location : NutsStoreLocation.values()) {
            ...
            if (NutsBlankable.isBlank(_storeLocation)) {
                switch (storeLocationStrategy) {
                    case STANDALONE: {
                        String c = NutsPlatformUtils.getCustomPlatformHomeFolder(platformOsFamily, location, homeLocations);
                ...
                switch (storeLocationStrategy) {
                    case STANDALONE: {
                        ...
                        storeLocations.put(location, c == null ?
                                (workspaceLocation + File.separator + location.id() + NutsReservedIOUtils.getNativePath("/" + _storeLocation))
                                :
                                (c + NutsReservedIOUtils.getNativePath("/" + _storeLocation)));
                        break;
                    }
                    case EXPLODED: {
                        storeLocations.put(location, homes[location.ordinal()] + NutsReservedIOUtils.getNativePath("/" + _storeLocation));
                        break;
                    }
                }
            } else {
