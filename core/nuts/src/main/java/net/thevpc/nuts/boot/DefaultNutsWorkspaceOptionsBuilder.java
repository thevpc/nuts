...
import net.thevpc.nuts.reserved.NutsReservedWorkspaceCommandLineParser;
...
//    private String bootRepositories;
    ...
    private NutsIsolationLevel isolationLevel;
    ...
    @Override
    public NutsWorkspaceOptionsBuilder setIsolationLevel(NutsIsolationLevel isolationLevel) {
    ...
    public NutsWorkspaceOptionsBuilder setOutputFormatOptions(List<String> options) {
        if (options != null) {
            ...
            return addOutputFormatOptions(NutsReservedCollectionUtils.nonNullList(options).toArray(new String[0]));
        } else {
    ...
    public NutsWorkspaceOptionsBuilder setStoreLocations(Map<NutsStoreLocation, String> storeLocations) {
        if (storeLocations != null) {
            ...
            this.storeLocations.putAll(NutsReservedCollectionUtils.nonNullMap(storeLocations));
        } else {
...
//        this.setExcludedRepositories(other.getExcludedRepositories() == null ? null : Arrays.copyOf(other.getExcludedRepositories(), other.getExcludedRepositories().length));
    ...
    public NutsWorkspaceOptionsBuilder setCommandLine(String[] args, NutsSession session) {
        NutsReservedWorkspaceCommandLineParser.parseNutsArguments(args, this, session);
        return this;
    }
    ...
    public NutsWorkspaceOptionsBuilder addOutputFormatOptions(String... options) {
        if (options != null) {
            for (String option : options) {
                if (option != null) {
                    option = NutsStringUtils.trim(option);
                    if (!option.isEmpty()) {
                        if (outputFormatOptions == null) {
                            outputFormatOptions = new ArrayList<>();
                        }
