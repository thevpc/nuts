...
public class NutsReservedClassLoaderUtils {
    ...
    public static URL findClassLoaderJar(NutsId id, URL[] urls) {
        for (URL url : urls) {
            NutsId[] nutsBootIds = NutsReservedMavenUtils.resolveJarIds(url);
                ...
                try {
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        if (!zname.endsWith("/") && zname.endsWith(".class") && !zname.contains("$")) {
                            if (NutsReservedUtils.isInfiniteLoopThread(NutsReservedClassLoaderUtils.class.getName(), "isLoadedClassPath")) {
                                return false;
                            }
