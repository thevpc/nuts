...
public class NSig {
    ...
    public static NSig of(Type... types) {
        return new NSig(types, false);
    }

    public static NSig ofVarArgs(Type... types) {
        checkVararg(types);
        return new NSig(types, true);
    }
    ...
    private NSig(Type[] types, boolean vararg) {
        this.types = types;
        this.vararg = vararg;
    }
    ...
    public NSig setVararg(boolean vararg) {
        if (vararg) {
            checkVararg(types);
        }
        Type[] types2 = Arrays.copyOfRange(types, 0, types.length);
        return new NSig(types2, vararg);
    }

    public NSig set(Type any, int pos) {
        Type[] types2 = Arrays.copyOfRange(types, 0, types.length);
        types2[pos] = any;
        if (vararg) {
            checkVararg(types2);
