...
public class DefaultNLocks extends AbstractNLocks {
    ...
    public NLock create() {
        ...
        if (lr == null) {
            ...
            Path p = toPath(s);
            if (p == null) {
                throw new NLockException(getSession(), NMsg.ofCstyle("unsupported lock for %s", s.getClass().getName()), null, s);
            }
            lrPath = p.resolveSibling(p.getFileName().toString() + ".lock");
        } else {
            lrPath = toPath(lr);
            if (lrPath == null) {
                throw new NLockException(getSession(), NMsg.ofCstyle("unsupported lock %s", lr.getClass().getName()), lr, s);
            }
        }
