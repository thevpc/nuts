...
public class MinimalYaml implements NutsElementStreamFormat {
        ...
        switch (c) {
            ...
            case '?': {
                ...
                if (current == ' ' || current == '\t') {
                    ...
                    return Node.forObjectElement(builder().forEntry().setKey(li).setValue(v).build());
                } else if (current == '\n' || current == ';') {
                    readNewLine();
                    int newIndent = peekIndent();
                    if (newIndent > indent) {
                ...
                if (current == ':') {
                    ...
                    if (readNewLine()) {
                        ...
                        if (newIndent > indent) {
                            ...
                            return Node.forObjectElement(builder().forEntry().setKey(li).setValue(builder().forPrimitive().buildString("")).build());
                        }
                    } else {
    ...
    public NutsElement parseElement(Reader reader, NutsElementFactoryContext context) {
        this.reader = (reader instanceof BufferedReader) ? ((BufferedReader) this.reader) : new BufferedReader(reader);
        readNext();
