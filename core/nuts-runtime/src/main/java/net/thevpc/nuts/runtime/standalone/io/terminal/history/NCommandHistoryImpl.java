 ...
 * @author thevpc
 */
public class NCommandHistoryImpl implements NCommandHistory {

    private NPath path;
    private final NSession session;
    private final List<NCommandHistoryEntry> entries = new ArrayList<>();

    public NCommandHistoryImpl(NSession session) {
                ...
                while ((line = out.readLine()) != null) {
                    if (line.length() > 0) {
                        if (line.startsWith("#")) {
                            ...
                            entries.add(new NCommandHistoryEntryImpl(index, line, instant));
                        }
                    }
                }
            } catch (IOException ex) {
    ...
    public void save(OutputStream outs) {
        try (PrintStream out = new PrintStream(outs)) {
            for (NCommandHistoryEntry entry : entries) {
    ...
    @Override
    public NCommandHistory setPath(Path path) {
        this.path = path == null ? null : NPath.of(path, session);
        return this;
    }

    @Override
    public NCommandHistory setPath(File path) {
        this.path = path == null ? null : NPath.of(path, session);
        return this;
    }

    @Override
    public NCommandHistory setPath(NPath path) {
        this.path = path;
        return this;
    }
    ...
    @Override
    public NCommandHistoryEntry getEntry(int index) {
        return entries.get(index);
    }

    @Override
    public ListIterator<NCommandHistoryEntry> iterator(int index) {
        return entries.listIterator(index);
    }

    @Override
    public void add(Instant time, String line) {
        entries.add(new NCommandHistoryEntryImpl(entries.size(), line, time));
    }
