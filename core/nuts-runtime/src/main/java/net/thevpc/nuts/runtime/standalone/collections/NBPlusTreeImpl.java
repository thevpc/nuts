...
public class NBPlusTreeImpl<K extends Comparable<K>, V> extends AbstractMap<K, V> implements NBPlusTree<K, V> {
    ...
    private void handleDeficiency(IntermediateNode<K, V> in) {
        ...
        if (in.size() == 0) {
            //okk
        } else if (!NBPlusTreeHelper.isDeficient(in)) {
            return;
        }
        ...
        if (NBPlusTreeHelper.eq(store.root(), in)) {
            ...
            for (int i = 0; i < childrenCount; i++) {
                store.addChild(sibling, in.child(i), -1);
            ...
            if (dpIndex < 0) {
                ...
                while (nn != null) {
                    ...
                    if (NBPlusTreeHelper.isEmpty(nn)) {
                        ...
                        store.free(nn);
                    } else if (NBPlusTreeHelper.isDeficient(nn)) {
                        if (nn.isLeaf()) {
                            LeafNode<K, V> ln = (LeafNode<K, V>) nn;
    ...
    public Set<Entry<K, V>> entrySet() {
        return new AbstractSet<Entry<K, V>>() {
            ...
            @Override
            public int size() {
                return NBPlusTreeImpl.this.size();
            }
        };
    }
...
//                    store.updateKeyAt(ln.parent(), ln.parent().degree() - 1, newParentKey);
    ...
    public List<V> search(K key) {
        ...
        while (true) {
            while (index > 0 && Objects.equals(ln.keyAt(index - 1), key)) {
                index--;
            }
            if (index == 0) {
                LeafNode<K, V> left = ln.leftSibling();
                if (left != null && left.size() > 0 && Objects.equals(left.keyAt(left.size() - 1), key)) {
                    ln = left;
                    index = left.size() - 1;
                    continue;
                }
