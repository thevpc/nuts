    ...
    private static class ElementParser {
        ...
        private NElement readLiteral(boolean asValue) {
            if (current == '"') {
                ...
                if (!str && sb.length() > 0) {
                    return NElement.ofNumber(trimmed);
//                    if (dot || E) {
//                        try {
        ...
        public NElement readElement(int indent) {
            ...
            switch (a.type) {
                case LITERAL:
                    return a.getElement();
                case OBJECT_ELEMENT: {
                    NObjectElementBuilder obj = NElement.ofObjectBuilder().add(a.getEntry());
                    while (true) {
                        readNewLine();
                ...
                case ARRAY_ELEMENT: {
                    NArrayElementBuilder arr = NElement.ofArrayBuilder().add(a.getElement());
                    while (true) {
                        readNewLine();
                        int newIndent = peekIndent();
                        if (newIndent < indent) {
            ...
            switch (c) {
                case '-': {
                    ...
                    if (current == ' ' || current == '\t') {
                        ...
                        if (newIndent > indent) {
                            ...
                            return Node.forArrayElement(NElement.ofString(""));
                        }
                    } else {
                        readerReset();
                        readNext();
                ...
                case '?': {
                    ...
                    if (current == ' ' || current == '\t') {
                        ...
                        return Node.forObjectElement(NElement.ofPair(li, v));
                    } else if (current == '\n' || current == ';') {
                        readNewLine();
                        int newIndent = peekIndent();
                        if (newIndent > indent) {
                    ...
                    if (current == ':') {
                        ...
                        if (readNewLine()) {
                            ...
                            if (newIndent > indent) {
                                ...
                                return Node.forObjectElement(NElement.ofPair(li, NElement.ofString("")));
                            }
                        } else {
