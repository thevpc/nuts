...
public class StyledParserStepOld extends ParserStep {
    ...
    public StyledParserStepOld(char c, boolean lineStart, NutsWorkspace ws, DefaultNutsTextNodeParser.State state) {
        start.append(c);
//        this.spreadLines = spreadLines;
    ...
    public StyledParserStepOld(String c, boolean lineStart, NutsWorkspace ws, DefaultNutsTextNodeParser.State state) {
        start.append(c);
//        this.spreadLines = spreadLines;
        this.lineStart = lineStart;
        this.ws = ws;
    ...
    public void consume(char c, DefaultNutsTextNodeParser.State state, boolean wasNewLine) {
        ...
        if (complete) {
            if (c == start.charAt(0)) {
                ...
                complete = false;
                state.applyPush(new StyledParserStepOld(
                        e2, false, ws, state
                ));
            ...
            if ((styleMode == StyleMode.COLON || styleMode == StyleMode.EMBEDDED) && !atPresentEnded) {
                ...
                if (styleMode == StyleMode.EMBEDDED && c == '}') {
                    ...
                    if (styleMode == StyleMode.EMBEDDED) {
                        if (wasSharp) {
                            wasSharp = false;
                            state.applyPush(new StyledParserStepOld("##", lineStart, ws, state));
                        } else {
                            wasSharp = true;
                        }
                    } else {
...
//                    state.applyPush(new StyledParserStep(c, spreadLines, false, ws));
                    ...
                    state.applyPush(new AntiQuote3ParserStep(c, lineStart, ws));
                } else {
                ...
                String y = end.readAll();
                if (y.length() > 1) {
                    state.applyPush(new StyledParserStepOld(y+c, lineStart, ws, state));
                } else {
    ...
    public NutsText toText() {
        ...
        if (styleMode == StyleMode.COLON) {
            ...
            String s=null;
            if(atPresentEnded){
                s=atStr.substring(1,atStr.length()-1);
            }else{
                s=atStr.substring(1,atStr.length()-1);
            }
