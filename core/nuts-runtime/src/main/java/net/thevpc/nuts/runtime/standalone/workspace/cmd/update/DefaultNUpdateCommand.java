 ...
 * @author thevpc
 */
public class DefaultNUpdateCommand extends AbstractNUpdateCommand {
    ...
    @Override
    public NUpdateCommand checkUpdates() {
        if (checkFixes) {
            checkFixes();
            traceFixes();
        }
...
//        NutsWorkspaceCurrentConfig jsonBootConfig = getConfigManager().getBootContext();
    ...
    public NUpdateCommand checkFixes() {
        resultFixes = null;
        checkSession();
        NWorkspace ws = getSession().getWorkspace();
        NWorkspaceExt dws = NWorkspaceExt.of(ws);
...
//        return se;
//    }
    private NFetchCommand latestOnlineDependencies(NFetchCommand se) {
