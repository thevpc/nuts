 ...
 * @author thevpc
 */
public class NElementSelectorFilters {
     ...
     * @return element path
     */
    public static NElementSelector compile(String jpath) {
        ...
        st.ordinaryChar('[');
        st.ordinaryChar(']');
        QueueJsonSelector q = new QueueJsonSelector();
        try {
            ...
            while (st.nextToken() != NToken.TT_EOF) {
                switch (st.ttype) {
                    ...
                    case '[': {
                        ...
                        if (p.isEmpty()) {
                            q.queue.add(new ArrItemCollectorJsonSelector());
                        } else {
                            q.queue.add(new SubItemJsonSelector(p));
                        }
                        break;
                    }
                    default: {
    ...
    private static class QueueJsonSelector implements NElementSelector {

        List<NElementSelector> queue = new ArrayList<>();
        ...
        public List<NElement> filter(List<NElement> elements) {
            for (NElementSelector jsonPath : queue) {
    ...
    private static class ArrItemCollectorJsonSelector implements NElementSelector {

        public ArrItemCollectorJsonSelector() {
        }
    ...
    private static class SubItemJsonSelector extends AbstractJsonSelector {

        private final String pattern;

        public SubItemJsonSelector(String subItem) {
            super();
    ...
    private static abstract class AbstractJsonSelector implements NElementSelector {

        public AbstractJsonSelector() {
