...
public class DefaultNFetchCommand extends AbstractNFetchCommand {
    ...
    public NDefinition fetchDefinitionNoCache(NId id, NFetchCommand options, boolean includeContent, boolean includeInstallInfo) {
            ...
            for (NRepositoryAndFetchMode location : descTracker.available()) {
                try {
                    ...
                    _LOGOP(getSession()).error(ex).level(Level.SEVERE)
                            .log(NMsg.ofJstyle("unexpected error while fetching descriptor for {0}", id));
                ...
                if (options.isEffective() || isDependencies()) {
                    try {
                        ...
                        _LOGOP(getSession()).level(Level.WARNING).verb(NLoggerVerb.WARNING)
                                .log(NMsg.ofJstyle("artifact descriptor found, but its parent is not: {0} with parent {1}", id.getLongName(),
