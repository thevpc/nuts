...
public class NDefinitionOsIdFilter extends AbstractDefinitionFilter {

    private Set<NId> accepted = new HashSet<>();

    public NDefinitionOsIdFilter() {
        super(NFilterOp.CUSTOM);
    }

    public NDefinitionOsIdFilter(Collection<NId> accepted) {
        super(NFilterOp.CUSTOM);
    ...
    public boolean acceptDefinition(NDefinition def) {
        List<String> current = NStream.ofIterable(def.descriptor().condition().os()).nonBlank().toList();
        if(current.isEmpty() || accepted.isEmpty()){
            return true;
        }
        for (NId nutsId : accepted) {
            if(CoreFilterUtils.matchesOs(nutsId.toString(),current)){
                return true;
            }
        }
        return false;
    ...
    public boolean equals(Object o) {
        ...
        NDefinitionOsIdFilter that = (NDefinitionOsIdFilter) o;
