...
public class DefaultNPrepareCmd extends AbstractNPrepareCmd {
    ...
    public NPrepareCmd run() {
        ...
        deps.addAll(NSearchCmd.of().addId("net.thevpc.nsh:nsh").setLatest(true).setContent(true).setTargetApiVersion(apiId.getVersion()).setDependencyFilter(NDependencyFilters.of().byRunnable()).setBasePackage(true).setDependencies(true).getResultIds().toList());
        if(ids!=null){
            for (NId id : deps) {
                deps.addAll(NSearchCmd.of().addId(id).setLatest(true).setContent(true).setTargetApiVersion(apiId.getVersion()).setDependencyFilter(NDependencyFilters.of().byRunnable()).setBasePackage(true).setDependencies(true).getResultIds().toList());
            }
        }
        for (NId dep : deps) {
    ...
    private void pushId(NId pid, NVersion apiIdVersion) {
        NDefinition def = NSearchCmd.of().addId(pid).setLatest(true).setContent(true).setTargetApiVersion(apiIdVersion).getResultDefinitions().findFirst().get();
