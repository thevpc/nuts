...
import net.thevpc.nuts.collections.NByteList;

import java.util.Arrays;
import java.util.List;

public class NByteArrayList implements NByteList {
    /**
    ...
    public NByteArrayList(int initialSize) {
        values = new byte[initialSize];
        size = 0;
    }

    public NByteArrayList() {
        values = new byte[0];
        size = 0;
    }

    public NByteArrayList(byte[] values, int offset, int size) {
        this.values = new byte[size];
        System.arraycopy(values, offset, this.values, 0, Math.min(this.values.length, size));
    }

    public NByteArrayList(byte[] values) {
        this(values,values.length);
    }

    public NByteArrayList(byte[] values, int size) {
...
//        modCount++;
    ...
    @Override
    public NByteArrayList subList(int offset, int count) {
        if (offset < 0) {
            count += offset;
            offset = 0;
        }
        ...
        return new NByteArrayList(this.values, offset, count);
    }
    ...
    @Override
    public NByteArrayList copy() {
        return new NByteArrayList(Arrays.copyOf(values, values.length), size);
    }
    ...
    @Override
    public NDoubleArrayList toDoubleList() {
    ...
    @Override
    public NIntArrayList toIntList() {
        int[] r = new int[size];
        for (int i = 0; i < size; i++) {
            r[i] = values[i];
        }
    ...
    @Override
    public int indexOf(byte o, int from) {
        return indexOf(o, from, size);
    }

    @Override
    public int indexOf(byte o, int from, int to) {
        byte[] a = this.values;
    ...
    @Override
    public boolean contains(byte o, int from) {
        return contains(o, from, size);
    }

    @Override
    public boolean contains(byte o, int from, int to) {
    ...
    @Override
    public int lastIndexOf(byte o, int from) {
        return lastIndexOf(o, from, size);
    }

    @Override
    public int lastIndexOf(byte o, int from, int to) {
        byte[] a = this.values;
        int max = Math.min(to, size);
