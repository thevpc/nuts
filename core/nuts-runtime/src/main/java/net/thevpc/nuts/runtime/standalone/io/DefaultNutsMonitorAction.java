...
//        LOG = ws.log().of(DefaultNutsMonitorAction.class);
    ...
    public NutsMonitorAction setSource(String path) {
        this.source = path == null ? null : NutsStreamOrPath.of(session.io().path(path));
        return this;
    }
    ...
    public InputStream create() {
        ...
        if (sourceName == null || sourceName.isEmpty()) {
            sourceName = session.text().toText(source);
        }
        if (sourceName == null || sourceName.isEmpty()) {
            sourceName = session.text().toText(source.getName());
        }
        if (sourceName == null || sourceName.isEmpty()) {
            sourceName = session.text().toText(String.valueOf(source.getValue()));
        }
