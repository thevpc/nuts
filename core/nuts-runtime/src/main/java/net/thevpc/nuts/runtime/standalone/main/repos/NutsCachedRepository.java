...
import net.thevpc.nuts.runtime.core.SuccessFailResult;
import net.thevpc.nuts.runtime.standalone.repocommands.AbstractNutsUpdateRepositoryStatisticsCommand;
import net.thevpc.nuts.runtime.standalone.util.common.CoreStringUtils;
import net.thevpc.nuts.runtime.standalone.util.io.CommonRootsHelper;
...
public class NutsCachedRepository extends AbstractNutsRepositoryBase {
    ...
    public final Iterator<NutsId> searchImpl(final NutsIdFilter filter, NutsFetchMode fetchMode, NutsSession session) {
        ...
        try {
            ...
            //ignore....
            LOG.with().level(Level.SEVERE).error(ex).log("Search latest versions error : {0}",CoreStringUtils.exceptionToString(ex));
        }
        if (p != null) {
            li.add(p);
        }
...
//                errors.append(ex).append(" \n");
        ...
        try {
            ...
            LOG.with().level(Level.SEVERE).error(ex).log("Search versions error : {0}",CoreStringUtils.exceptionToString(ex));
            //ignore....
        }
