    ...
    public void setConfig(NutsRepositoryConfig newConfig, NutsSession session, boolean fireChange, NutsUpdateOptions options) {
        ...
        if (config.getMirrors() != null) {
            for (NutsRepositoryRef ref : config.getMirrors()) {
                NutsRepository r = ((DefaultNutsRepositoryManager)repository.getWorkspace().repos()).createRepository(CoreNutsUtils.refToOptions(ref), getMirrorsRoot(), repository);
...
//    }
    ...
    public NutsRepository addMirror(NutsAddRepositoryOptions options) {
        ...
        NutsRepository repo = ((DefaultNutsRepositoryManager)repository.getWorkspace().repos()).createRepository(options, getMirrorsRoot(), repository);
