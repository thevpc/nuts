...
public class DefaultNutsWorkspaceLocationModel {
    ...
    public void setHomeLocation(NutsOsFamily layout, NutsStoreLocation folder, String location, NutsSession session) {
        if (folder == null) {
            throw new NutsIllegalArgumentException(session, "invalid store root folder null");
        }
...
//        }
    ...
    public String getDefaultIdContentExtension(String packaging,NutsSession session) {
        if (CoreStringUtils.isBlank(packaging)) {
            throw new NutsIllegalArgumentException(session, "unsupported empty Packaging");
        }
        switch (packaging) {
            case "jar":
            case "bundle":
        ...
        switch (f) {
            ...
            default: {
                ...
                if (CoreStringUtils.isBlank(f)) {
                    throw new NutsIllegalArgumentException(session, "missing face in " + id);
                }
                throw new NutsIllegalArgumentException(session, "unsupported face " + f + " in " + id);
            }
        }
    }
