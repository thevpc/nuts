...
public class NLogBoot {
    ...
    public NLogBoot(NBootOptionsBoot bootTerminal) {
    ...
    public void log(Level lvl, String logVerb, NMsgBoot message) {
    ...
    public void log(Level level, String verb, Supplier<NMsgBoot> msgSupplier, Supplier<Throwable> errorSupplier) {
...
//        }
        ...
        log(level, verb, NMsgBoot.ofPlain(record.getMessage()), record.getThrown());
    }
    ...
    public void log(Level lvl, String logVerb, NMsgBoot message, Throwable err) {
        if (isLoggableTerm(lvl)) {
            doLogTerm(lvl, logVerb, message == null ? "" : message.toString());
            if (err != null) {
    ...
    public void setOptions(NBootOptionsBoot options) {
        ...
        NLogConfigBoot nLogConfig = options.getLogConfig();
        if (nLogConfig != null) {
...
//        String rootPackage = "net.thevpc.nuts";
                ...
                int MEGA = 1024 * 1024;
                if (name == null || NStringUtilsBoot.isBlank(name)) {
                    ...
                    switch (c) {
                        case '%': {
                            ...
                            switch (charArray[i]) {
                                ...
                                case 't': {
                                    String tempFolder = Paths.get(NPlatformHomeBoot.of(options.getStoreLayout()).getWorkspaceStore("TEMP", options.getWorkspace())).toString();
                ...
                if (folder == null || NStringUtilsBoot.isBlank(folder)) {
                    String logFolder = Paths.get(NPlatformHomeBoot.of(options.getStoreLayout()).getWorkspaceStore("LOG", options.getWorkspace())).toString();
                    folder = logFolder + "/" + NConstants.Folders.ID + "/net/thevpc/nuts/nuts/" + NBootWorkspace.getVersion();
                }
    ...
    public void outln(String msg, Object... p) {
        this.out.println(NMsgBoot.ofC(msg, p));
        this.out.flush();
    }
    ...
    public static class LogOp {
        private NLogBoot logger;
        private Level level = Level.FINE;
        private String verb;
        private NMsgBoot msg;
        private long time;
        private Supplier<NMsgBoot> msgSupplier;
        private Throwable error;

        public LogOp(NLogBoot logger) {
            this.logger = logger;
        }
        ...
        public void log(NMsgBoot message) {
            this.msg = message;
            run();
        }


        public void log(Supplier<NMsgBoot> msgSupplier) {
