 ...
 * @since 0.8.3
 */
public class NHomeLocationBoot  {
    private static final Map<String, NHomeLocationBoot> CACHE = new HashMap<>();
    private final String osFamily;
    private final String storeLocation;

    private NHomeLocationBoot(String osFamily, String storeLocation) {
        this.osFamily = osFamily;
        this.storeLocation = storeLocation;
    }

    public static NHomeLocationBoot of(String osFamily, String storeLocation) {
        String key = (osFamily == null ? "system" : osFamily) + "_" + (storeLocation == null ? "system" : storeLocation);
        NHomeLocationBoot instance = CACHE.get(key);
        if (instance == null) {
            synchronized (CACHE) {
                instance = CACHE.get(key);
                if (instance == null) {
                    instance = new NHomeLocationBoot(osFamily, storeLocation);
    ...
    public static NHomeLocationBoot parse(String value) {
        if (value == null) {
            value = "";
        } else {
        ...
        if (i >= 0) {
            ...
            if (osf == null) {
                if (!s1.equals("system") && s1.length() > 0) {
                    throw new NBootException(NMsgBoot.ofC(NHomeLocationBoot.class.getSimpleName() + " invalid value : %s", finalValue));
                }
            }
            if (loc == null) {
        ...
        throw new IllegalArgumentException(NMsgBoot.ofC(NHomeLocationBoot.class.getSimpleName() + " invalid value : %s", finalValue).toString());
    }
    ...
    public boolean equals(Object o) {
        ...
        NHomeLocationBoot that = (NHomeLocationBoot) o;
     ...
     */
    public String name() {
        return ((osFamily == null ? "system" : osFamily) + "_" + (storeLocation == null ? "system" : storeLocation)).toUpperCase();
    }
     ...
     */
    public String id() {
        return (osFamily == null ? "system" : osFamily) + "-" + (storeLocation == null ? "system" : storeLocation);
    }
}
