...
public class NWorkspaceCmdLineFormatterBoot {
    ...
    private NWorkspaceOptionsConfigBoot config;
    private NBootOptionsBoot options;

    public NWorkspaceCmdLineFormatterBoot(NWorkspaceOptionsConfigBoot config, NBootOptionsBoot options) {
        this.config = config;
        ...
        if (value != null) {
            if (config.isShortOptions()) {
                if ("NOsFamily".equals(enumType)) {
                    switch (NUtilsBoot.enumName(value)) {
                        case "LINUX": {
    ...
    private void fillOptionEnum(String value, String enumType, List<String> arguments, boolean forceSingle) {
        if (value != null) {
            ...
            arguments.add("--" + NUtilsBoot.enumId(value));
        }
    }
...
//        NVersionBoot apiVersionObj = config.getApiVersion();
        ...
        if (NStringUtilsBoot.isBlank(wsString)) {
...

        {
            ...
            if (!isApiVersionOrAfter(V084)) {
                if (NUtilsBoot.firstNonNull(options.getBot(), false)) {
                    //force filtered for older nuts
                    nTerminalMode = "FILTERED";
                }
            }
            if (!(config.isOmitDefaults() && NUtilsBoot.sameEnum(nTerminalMode, "FORMATTED"))) {
        ...
        fillOption("--skip-welcome", "-K", NUtilsBoot.firstNonNull(options.getSkipWelcome(), false), false, arguments, false);
        ...
        if (options.getFetchStrategy() != null && NUtilsBoot.sameEnum(options.getFetchStrategy(), "ONLINE")) {
        ...
        fillOption("--name", null, NStringUtilsBoot.trim(options.getName()), arguments, false);
        ...
        for (String location : NPlatformHomeBoot.storeTypes()) {
            String s = storeLocations.get(location);
            if (!NStringUtilsBoot.isBlank(s)) {
                fillOption("--" + NUtilsBoot.enumId(location) + "-location", null, s, arguments, false);
            }
        }

        Map<NHomeLocationBoot, String> homeLocations = options.getHomeLocations();
        if (homeLocations != null) {
            for (String location : NPlatformHomeBoot.storeTypes()) {
                String s = homeLocations.get(NHomeLocationBoot.of(null, location));
                if (!NStringUtilsBoot.isBlank(s)) {
                    fillOption("--system-" + NUtilsBoot.enumId(location) + "-home", null, s, arguments, false);
                }
            }
            for (String osFamily : NPlatformHomeBoot.osFamilies()) {
                for (String location : NPlatformHomeBoot.storeTypes()) {
                    String s = homeLocations.get(NHomeLocationBoot.of(osFamily, location));
                    if (!NStringUtilsBoot.isBlank(s)) {
                        fillOption("--" + NUtilsBoot.enumId(osFamily) + "-" + NUtilsBoot.enumId(location) + "-home", null, s, arguments, false);
                    }
                }
            }
        }
        ...
        fillOption("--help", "-h", NUtilsBoot.firstNonNull(options.getCommandHelp(), false), false, arguments, false);
        fillOption("--version", "-v", NUtilsBoot.firstNonNull(options.getCommandVersion(), false), false, arguments, false);

        if (!(config.isOmitDefaults() && (options.getOpenMode() == null) || NUtilsBoot.sameEnum(options.getOpenMode(), "OPEN_OR_CREATE"))) {
        ...
        return new NCmdLineBoot(arguments);
    }
    ...
    private boolean isApiVersionOrAfter(String version) {
        ...
        NVersionBoot apiVersionObj = NVersionBoot.of(apiVersionStr);
