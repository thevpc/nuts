...
public class NVersionBoot  {
    ...
    public static NVersionBoot BLANK = new NVersionBoot("");

    private static final long serialVersionUID = 1L;
    protected String expression;
    private VersionParts parts;

    public NVersionBoot(String expression) {
        this.expression = (NStringUtilsBoot.trim(expression));
    }
    ...
    public static int compareVersions(String v1, String v2) {
        v1 = NStringUtilsBoot.trim(v1);
        v2 = NStringUtilsBoot.trim(v2);
        if (v1.equals(v2)) {
            return 0;
        }
    ...
    public static NVersionBoot of(String value) {
        return new NVersionBoot(value);
    }
    ...
    public int compareTo(NVersionBoot other) {
...
//    }
    ...
    public NVersionBoot compatNewer() {
        String v = toExplicitSingleValueOrNullString();
        if (v == null) {
            return this;
        }
        return new NVersionBoot("[" + expression + ",[");
    }

    
    public NVersionBoot compatOlder() {
        String v = toExplicitSingleValueOrNullString();
        if (v == null) {
            return this;
        }
        return new NVersionBoot("]," + v + "]");
    }
    ...
    public String asSingleValue() {
        ...
        if (seps == 0) {
            if (commas == 0) {
                ...
                Set<String> all = new HashSet<>(NStringUtilsBoot.split(s, ",", true, true));
                if (all.size() == 1) {
            ...
            if (o == '[' && c == ']') {
                if (commas == 0) {
                    ...
                    //commas==1
                    Set<String> two = new HashSet<>(NStringUtilsBoot.split(s, ",", true, false));
                    if (two.size() == 1) {
    ...
    public NVersionBoot inc() {
        return inc(-1);
    }

    
    public NVersionBoot inc(int index) {
        return inc(index, 1);
    }

    
    public NVersionBoot inc(int index, long amount) {
        return new NVersionBoot(incVersion(getValue(), index, amount));
    }

    
    public NVersionBoot inc(int index, BigInteger amount) {
        return new NVersionBoot(incVersion(getValue(), index, amount));
    }
    ...
    public boolean equals(Object o) {
        ...
        NVersionBoot version = (NVersionBoot) o;
    ...
    public static class NVersionIntervalBoot implements Serializable {
        ...
        static NVersionIntervalBoot of(String s) {
            ...
            if (x.size() > 1) {
                throw new NBootException(NMsgBoot.ofC("too many intervals"));
            }
            return x.get(0);
        }
        ...
        public NVersionIntervalBoot(boolean inclusiveLowerBoundary, boolean inclusiveUpperBoundary, String min, String max) {
            ...
            this.lowerBound = NStringUtilsBoot.trimToNull(min);
            this.upperBound = NStringUtilsBoot.trimToNull(max);
        }

        public boolean acceptVersion(NVersionBoot version) {
            if (!NStringUtilsBoot.isBlank(lowerBound) && !lowerBound.equals(NConstants.Versions.LATEST) && !lowerBound.equals(NConstants.Versions.RELEASE)) {
                int t = version.compareTo(lowerBound);
                if ((includeLowerBound && t < 0) || (!includeLowerBound && t <= 0)) {
                    return false;
                }
            }
            if (!NStringUtilsBoot.isBlank(upperBound) && !upperBound.equals(NConstants.Versions.LATEST) && !upperBound.equals(NConstants.Versions.RELEASE)) {
        ...
        public boolean isFixedValue() {
            return includeLowerBound && includeUpperBound && NStringUtilsBoot.trim(lowerBound).equals(NStringUtilsBoot.trim(upperBound))
        ...
        void addNextValue(String sval) {
            if (sval.endsWith("*")) {
                ...
                if (min.equals("")) {
                    ...
                    String max = NVersionBoot.of(min).inc(-1).getValue();
                    ...
                    switch (state) {
                        case NEXT: {
                            switch (t) {
                                ...
                                default: {
                                    throw new NBootException(NMsgBoot.ofC("unexpected  %s", ((char) t)));
                                }
                            }
                            break;
                        }
                        case NEXT_COMMA: {
                            switch (t) {
                                ...
                                default: {
                                    throw new NBootException(NMsgBoot.ofC("unexpected ',' found %s", ((char) t)));
                                }
                            }
                            break;
                        }
                        ...
                        default: {
                            throw new NBootException(NMsgBoot.ofC("unsupported state %s", state));
                        }
                    }
                }
                if (state != NEXT_COMMA && state != NEXT) {
                    throw new NBootException(NMsgBoot.ofC("invalid state %s", state));
                }
            } catch (IOException ex) {
                throw new NBootException(NMsgBoot.ofC("parse version failed: %s : ", version, ex));
            }
            return dd;
        }
    }
