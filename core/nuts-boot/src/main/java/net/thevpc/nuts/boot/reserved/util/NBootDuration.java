...
public class NBootDuration implements Serializable {
    ...
    public NBootDuration withSmallestUnit(ChronoUnit smallestUnit) {
        NBootDuration d = new NBootDuration(toUnitsArray(), smallestUnit, largestUnit);
    ...
    public NBootDuration normalize() {
        ...
        NBootDuration d = new NBootDuration(values, smallestUnit, largestUnit);
        if (this.timeMillis != d.timeMillis || this.timeNanos != d.timeNanos) {
            throw new IllegalArgumentException("unexpected");
        }
        return d;
