...
public class SqlCreateIndexCmd<C extends NdbConfig> extends CreateIndexCmd<C> {
    public SqlCreateIndexCmd(SqlSupport<C> support, String... names) {
        super(support, names);
    }
    @Override
    ...
    protected void runCreateIndex(ExtendedQuery eq, C options) {
        ...
        sql.append("create index ").append(eq.getTable()).append(" (");
        sql.append(setKeys);
        sql.append(") values (");
        sql.append(setVals);
        sql.append(")");
