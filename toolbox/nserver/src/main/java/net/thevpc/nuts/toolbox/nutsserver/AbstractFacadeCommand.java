...
public abstract class AbstractFacadeCommand implements FacadeCommand {
    ...
    public void execute(FacadeCommandContext context) throws IOException, LoginException {
        ...
        NWorkspaceSecurityManager secu = NWorkspaceSecurityManager.of(context.getSession());
        ...
        if (!NBlankable.isBlank(userLogin)) {
            ...
            try {
                NWorkspaceSecurityManager.of(context.getSession()).login(userLogin, userPassword);
                loggedId = true;
                executeImpl(context);
            } finally {
                if (loggedId) {
