...
public class ChmodCommand extends AbstractNutsCommand {
    ...
    protected void chmod(File f, Mods m, NutsPrintStream out) {
        System.out.println("chmod "+f+" :: "+m);
        if (m.r == 0 && m.w == 0 && m.x == 0) {
            return;
        }
        if (m.r != 0) {
            if(!f.setReadable(m.r == 1, m.user)){
                out.printf("Unable to "+((m.r == 1)?"set":"unset")+" read  flag for "+f);
            }
        }
        if (m.w != 0) {
            if(!f.setWritable(m.w == 1, m.user)){
                out.printf("Unable to "+((m.w == 1)?"set":"unset")+" write flag for "+f);
            }
        }
        if (m.x != 0) {
            if(!f.setExecutable(m.x == 1, m.user)){
                out.printf("Unable to "+((m.x == 1)?"set":"unset")+" exec  flag for "+f);
            }
        }
        if (f.isDirectory()) {
