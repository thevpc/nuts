...
public class TableFormatter {
    ...
    private DefaultCellFormatter defaultFormatter;
    /**
     * ABBBBCBBBBD
     * E    F    G
     * HIIIIJIIIIK
...
//    }
    ...
    public TableFormatter(NutsWorkspace ws) {
        this.ws = ws;
        defaultFormatter = new DefaultCellFormatter(ws);
    }
    ...
    public String toString() {
        ...
        if (rows.size() > 0) {
            List<Cell> cells = this.rows.get(0).cells;
            if (
                ...
                for (int i = 0; i < cells.size(); i++) {
                    ...
                    sb.append(StringUtils.fillString(B, s.length()));
                }
...
//                }
    ...
    private static class DefaultCellFormatter implements CellFormatter {
        private NutsWorkspace ws;

        public DefaultCellFormatter(NutsWorkspace ws) {
            this.ws = ws;
        }
    ...
    public boolean configure(CommandLine cmdLine) {
        if (cmdLine.read("--no-header")) {
            setVisibleHeader(false);
            return true;
        } else if (cmdLine.read("--header")) {
            setVisibleHeader(true);
            return true;
        } else if (cmdLine.read("--border")) {
            switch (cmdLine.readValue()) {
                case "spaces": {
            ...
            for (Map.Entry<String, Integer> e : columns.entrySet()) {
                if (cmdLine.read("--" + e.getKey())) {
                    setVisibleColumn(e.getValue(), true);
                    return true;
                } else if (cmdLine.read("--no-" + e.getKey())) {
