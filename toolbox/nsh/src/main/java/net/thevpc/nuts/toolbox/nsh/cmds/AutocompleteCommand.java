...
public class AutocompleteCommand extends SimpleNshBuiltin {
    ...
    protected void createResult(NutsCommandLine commandLine, SimpleNshCommandContext context) {
        Options options = context.getOptions();
        if (options.cmd == null) {
            throw new NutsExecutionException(context.getSession(), "missing JShellCommandNode", 1);
        }
        if (options.index < 0) {
            options.index = options.items.size();
            options.items.add("");
    ...
    protected void printPlainObject(SimpleNshCommandContext context, NutsSession session) {
        ...
        for (String o : new TreeSet<String>((Set) p.keySet())) {
            if (o.startsWith("-")) {
                // option
                context.out().printf("%s\n", text.text().forStyled(o,NutsTextNodeStyle.primary(4)));
            } else if (o.startsWith("<")) {
                context.out().printf("%s\n", text.text().forStyled(o,NutsTextNodeStyle.primary(1)));
            } else {
                context.out().printf("%s\n",
                        text.text().forStyled(o,NutsTextNodeStyle.pale())
                );
            }
        }
    }
