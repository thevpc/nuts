...
import net.thevpc.nuts.toolbox.nadmin.util.NonBlockingInputStreamAdapter;
...
public class ConnectNAdminSubCommand extends AbstractNAdminSubCommand {
    ...
    public boolean exec(NutsCommandLine commandLine, Boolean autoSave, NutsApplicationContext context) {
        ...
        if (commandLine.next("connect") != null) {
            ...
            int port = -1;
            if (server == null) {
                throw new NutsIllegalArgumentException(context.getSession(), NutsMessage.cstyle("missing address"));
            }
            if (server.contains("@")) {
            ...
            if (!_StringUtils.isBlank(login) && isBlank(password)) {
            ...
            try {
                try {
                    ...
                    _IOUtils.pipe("pipe-out-socket-" + server + ":" + validPort, new NonBlockingInputStreamAdapter("pipe-out-socket-" + server + ":" + validPort, socket.getInputStream()), context.getSession().out().asPrintStream());
                    PrintStream out = new PrintStream(socket.getOutputStream());
                    if (!_StringUtils.isBlank(login)) {
