            ...
            if (context.getSession().isPlainTrace()) {
                ...
                for (NdiScriptnfo ndiScriptnfo : result) {
                    context.getSession().out().printf("%s script %-" + namesSize + "s for "
                            ...
                            ? ws.formats().text().forStyled("re-installing", NutsTextNodeStyle.success())
                            : ws.formats().text().forStyled("installing", NutsTextNodeStyle.success()),
                            ndiScriptnfo.getName(),
                            ws.formats().text().forStyled(NdiUtils.betterPath(ndiScriptnfo.getPath().toString()), NutsTextNodeStyle.path())
                    );
                }
