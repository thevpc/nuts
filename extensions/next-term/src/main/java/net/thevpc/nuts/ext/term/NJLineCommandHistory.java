 ...
 * @author thevpc
 */
public class NJLineCommandHistory implements NCommandHistory {
    ...
    public NJLineCommandHistory(NSession session) {
...
//        }
    ...
    protected void trimHistory(Path path, int max) throws IOException {
        ...
        try (BufferedReader reader = Files.newBufferedReader(path)) {
            reader.lines().forEach(l -> {
                ...
                Instant time = Instant.ofEpochMilli(Long.parseLong(l.substring(0, idx)));
                String line = unescape(l.substring(idx + 1));
                allItems.add(new NJLineCommandHistoryEntry(allItems.size(), time, line));
            });
        }
        // Remove duplicates
    ...
    protected void internalAdd(Instant time, String line) {
        ...
        History.Entry entry = new NJLineCommandHistoryEntry(index, time, line);
        items.add(entry);
        maybeResize();
    }
...
//        index = size();
    ...
    public NCommandHistoryEntry getEntry(final int index) {
        return (NCommandHistoryEntry) items.get(index);
    }
