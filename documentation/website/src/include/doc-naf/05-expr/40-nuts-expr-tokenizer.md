...
| One fixed pair of quote characters can be configured, and there's no interpolated-string concept — a quoted string is always just a plain string. | Three quote characters are pre-registered by default (`"`, `'`, `` ` ``), and a **`$`-prefixed** variant of each is recognized as a distinct *interpolated*-string token type (`TT_ISTR_SQ`, `TT_ISTR_DQ`, `TT_ISTR_AQ` for `$'...'`, `$"..."`, `` $`...` `` respectively) — this is the lexical foundation for NExpr's `$`-interpolated string literals (see [`NExpr-Templating.md §4`](./NExpr-Templating.md)), distinguished at the *tokenizer* level so the parser can build an `NExprInterpolatedStringNode` directly instead of post-processing a plain string for embedded `$var` references. |
