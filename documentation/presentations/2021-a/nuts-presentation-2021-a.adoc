...
image::images/java-error.png[scaledwidth=70%]
...
* No Shared Dependencies / Centralize Dep Mgt
* What about `trivrost`, `OpenJNLP` ?
...
* We can run a gui app of course, `nuts` will create a shortcut for it

image::images/nuts-install-nbl.png[scaledwidth=70%]
...
* using `--json`,  `--xml`, `--props`, `--yaml` , `--table`  , `--tree` for any command will customize output

image::images/nuts-simple-format.png[scaledwidth=70%]
