    ...
    public synchronized ClassLoader resolveNutsClassLoader() {
        if (nutsClassLoader == null) {
            ...
            try {
                ...
                try {
                    nutsClassLoader = resolveNutsWorkspace().find().ids(pathList).mainAndDependencies().getResultClassLoader();
                } catch (Exception ex) {
                    ex.printStackTrace();
...
//        WebResource resource = null;
    ...
    private File resolveFileCodeBase(URL url) {
        ...
        if (separatorIndex != -1) {
            ...
            throw new NutsIllegalArgumentException("Unable to resolve url from " + urlFile);
        }
    }
