...
public class Test05_FindTest {
    ...
    public void find1() throws Exception {
        List<NId> def = NSearchCommand.of(session).addId("nuts").setOptional(false).setLatest(true).setFailFast(false)
//                .repository("maven-local")
    ...
    @Test()
    public void find2() {
        NStream<NId> result = NSearchCommand.of(session)
    ...
    public void find4() throws Exception {
        ...
        List<NId> result1 = NSearchCommand.of(session).setLatest(true).addId("nuts-runtime").getResultIds().toList();
        List<NId> result2 = NSearchCommand.of(session).setLatest(false).addId("nuts-runtime").getResultIds().toList();
    ...
    public void find5() throws Exception {
        List<NId> result1 = NSearchCommand.of(session).configure(false, "nuts-runtime").getResultIds().toList();
        List<NId> result2 = NSearchCommand.of(session).configure(false, "--latest", "nuts-runtime").getResultIds().toList();
        TestUtils.println("=====================");
        TestUtils.println(result1);
        TestUtils.println("=====================");
        TestUtils.println(result2);
...
//                "netbeans-launcher#1.1.0"
    ...
    public void find7() throws Exception {
        NStream<NId> resultIds = NSearchCommand.of(session).addId("net.thevpc.scholar.doovos.kernel:doovos-kernel-core")
